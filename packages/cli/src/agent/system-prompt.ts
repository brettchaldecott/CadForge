...
export function buildSystemPrompt(
  ...
  const parts = [
    ...
    '',
    '  This runs multiple LLM models in parallel, cross-critiques proposals,',
    '  sandbox-evaluates each, scores fidelity against the golden specification,',
    '  and selects or merges the best result.',
    '',
    '  To refine a previous competitive design (add features, fix issues):',
    '',
    '    Task({ agent_type: "competitive", prompt: "<change request>", design_id: "<id>" })',
    '',
    '  The pipeline will load the previous design\'s code and spec as the starting point,',
    '  and treat the prompt as a change request rather than a new design.',
    '  The design_id is returned in the competitive_status events from previous runs.',
    '',
    '  Use the competitive pipeline when:',
    '  - The user asks for "competitive" or "multi-model" design',
    '  - The design has tight dimensional tolerances',
    '  - The user wants to refine an existing competitive design',
    '',
